...
class Hyper_Network(nn.Module):
    ...
    def __init__(self,meta_v_dim, base_v_dim, output_dim, is_policy):
        ...
        if self.is_policy:
            self.last_layer_log = Head(z_dim, dynamic_layer, output_dim,  sttdev=0.001)
    ...
    def forward(self, meta_v, base_v, debug=None):
        ...
        mu = torch.bmm(w2, out) * s2 + b2 

        if self.is_policy:
            w3, b3, s3 = self.last_layer_log(z)
            log_std = torch.bmm(w3, out) * s3 + b3

            return mu.view(-1, self.output_dim), log_std.view(-1, self.output_dim)

        return mu.view(-1, self.output_dim)
