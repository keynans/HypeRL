...
class MetaLearner(object):
    ...
    def sample(self, tasks, scale, first_order=False, deterministic=False):
        ...
        for task in tasks:
            ...
            valid_episodes = self.sampler.sample(task_tensor, self.policy, deterministic, params=params,
                gamma=self.gamma, device=self.device)
    ...
    def kl_divergence(self, episodes, old_pis=None):
        ...
        for (task, train_episodes, valid_episodes), old_pi in zip(episodes, old_pis):
            ...
            if self.policy.is_hyper:
                z = self.policy.hyper.get_z(task)
            pi, _ = self.policy(valid_episodes.observations, task, params=params, z=z)
    ...
    def evaluate_task(self, unseen_task, scale, episode_num, gradient_steps=1, deterministic=False):
        episode = self.sample(unseen_task, scale, deterministic)
